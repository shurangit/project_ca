   ...
   /* md_addr_t *repl_addr = (md_addr_t *)malloc(sizeof(md_addr_t)); // Here we need to add last replaced block/address to the cache structure
    if (victim_cache){
	 ...
	 if (cache_dl2){ // L1 Missed so check for data in L2
        int j = 0;
       j = cache_access(cache_dl2, cmd, baddr, NULL, bsize, now, NULL, repl_addr);
    ...
    return cache_access(cache_dl2, cmd, baddr, NULL, bsize, now, NULL, NULL); // Normal access fn
   } else {
	...
	if (victim_cache){
        if (cache_il1->last_blk_addr != 0){
            if (cache_probe(victim_cache, baddr) != 0){ // Hit in VC
                ...
                cache_il1->vc_hits++;
                cache_flush_addr(victim_cache, baddr, now); // Swap this "hit" with the replacement
                cache_access(victim_cache, 0, cache_il1->last_blk_addr, NULL, bsize, now, NULL, NULL); // Add replaced block to victim cache and return
                return 0;
            } else {
                cache_il1->vc_misses++;
                cache_access(victim_cache, 0, cache_il1->last_blk_addr, NULL, bsize, now, NULL, NULL); // Add replaced block to victim cache and continue
            }
        }
    } 
	 if (cache_il2){ // L1 Missed so check for data in L2
       int j = 0;
        j = cache_access(cache_il2, cmd, baddr, NULL, bsize, now, NULL, repl_addr);
        if (j != 1) printf("CAT: %d", j);
       return j;
   /} else {
    ...
    if (cache_il2) {
      ...
      return cache_access(cache_il2, cmd, baddr, NULL, bsize, now, NULL, NULL); // Normal access fn
    } else {
	...
	fatal("bad l1 D-cache parms: <name>:<nsets>:<bsize>:<assoc>:<repl>");
      cache_dl1 = cache_create(name, nsets, bsize, /* balloc */FALSE,
       ...
       /* if (!victim_cache_opt){
            ...
            victim_cache = cache_create("vc", 2, bsize, FALSE,
            0, 16, cache_char2policy(c), zero_access_fn, 0);
        }*/
		 /* use an victim cache? */
       if (!stream_buffer_opt){
            stream_buffer = NULL;
       } else {
          stream_buffer = cache_create("sb", 1, bsize, FALSE,
           0, 4, cache_char2policy('f'), il2_access_fn, 1);
	  }
